        ...
        <div className={`relative overflow-hidden rounded-2xl border transition-all ${
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {isLive && (
              <span className="flex items-center gap-1.5 px-3 py-1 bg-red-500 text-white text-xs font-bold rounded-full">
                ...
                {match.status === 'halftime' ? 'HT' : `LIVE ${match.minute}'`}
              </span>
            )}
            {isFinished && (
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex-1 text-center">
              <div className={`w-16 h-16 mx-auto mb-3 rounded-xl bg-gray-50 flex items-center justify-center ${
                variant === 'compact' ? 'w-12 h-12' : ''
              }`}>
                {match.home_logo ? (
                  <img src={match.home_logo} alt="" className={`${variant === 'compact' ? 'w-8 h-8' : 'w-12 h-12'} object-contain`} />
                ) : (
            ...
            <div className="px-6">
              {isScheduled ? (
                <div className="text-center">
                  <span className="text-3xl font-black text-gray-300">VS</span>
                </div>
              ) : (
                <div className="flex items-center gap-3">
                  <span className={`${variant === 'compact' ? 'text-2xl' : 'text-4xl'} font-black ${
                    isLive ? 'text-[#0a1628]' : 'text-[#0a1628]'
            ...
            <div className="flex-1 text-center">
              <div className={`w-16 h-16 mx-auto mb-3 rounded-xl bg-gray-50 flex items-center justify-center ${
                ...
                {match.away_logo ? (
                  <img src={match.away_logo} alt="" className={`${variant === 'compact' ? 'w-8 h-8' : 'w-12 h-12'} object-contain`} />
                ) : (
