...
export default function NewsGrid({ news = [], title = "Latest News", showViewAll = true }) {
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex items-center justify-between mb-10">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-3xl font-black text-[#1B2852]">{title}</h2>
          </div>
          {showViewAll && (
            <Link 
                ...
                <div className="relative h-[400px] rounded-2xl overflow-hidden">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-8">
                    <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold mb-4 ${categoryColors[featuredNews.category] || 'bg-gray-500 text-white'}`}>
                      {featuredNews.category?.replace(/_/g, ' ').toUpperCase()}
                    </span>
                    ...
                    <div className="flex items-center gap-4 text-white/60 text-sm">
                      <span className="flex items-center gap-1">
                        ...
                        {featuredNews.published_at ? format(new Date(featuredNews.published_at), 'MMM d, yyyy') : 'Recent'}
                      </span>
